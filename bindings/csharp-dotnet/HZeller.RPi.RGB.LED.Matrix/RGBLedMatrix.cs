...
namespace rpi_rgb_led_matrix_sharp
{
    ...
    {
        #region DLLImports
        [DllImport("librgbmatrix.so.1")]
        internal static extern IntPtr led_matrix_create(int rows, int chained, int parallel);

        [DllImport("librgbmatrix.so.1", CallingConvention= CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
        ...
        public RGBLedMatrix(RGBLedMatrixOptions options, params string[] extraArgs)
        {
            ...
            try {
                ...
                var slowdownArg = "--led-slowdown-gpio="+(options.GpioSlowdown == 0 ? 1 : options.GpioSlowdown);
                ...
                // the commandline.
                var argv = new List<string> { appName, slowdownArg }.Concat(extraArgs).ToArray();
